...
import styles    from './SidOneCom.module.css';   /** This import is the custom CSS file that contains all of the styling declarations for this component. */
...
function SidOneCom ( props : SidOneComPro ) : React.ReactElement {
    ...
    */
    ...
    /* Sidebar One Paragraph Content = This custom variable stores the string that will be displayed in this component's .sidebarOneParagraph HTML element based on the current state of the mock data fetching process. If the data is currently being fetched, it will display a loading message. If an error was encountered during fetching, it will display an error message. Otherwise, it will display the actual mock back end JSON data string. */
    const sb1ParCon : string         = curLoaBoo ? 'Fetching data from API...' : encErrBoo ? 'Error fetching data' : mocDatObj.mocDatStr;
    ...
    */
    ...
    const sidOneComJsx : React.ReactElement = (
        ...
        <  div id='sb1DivEle' className={ styles.sidebarOneDiv } > { /* Sidebar One Div Element = This custom section element is the root HTML element and container for this component since React requires the JSX to return a single root element. */ }
            ...
            < p  id='sb1ParEle' className={ styles.sidebarOneParagraph } > { /* Sidebar One Paragraph Element = This custom paragraph element is the container for the mock backend JSON data. */ }

                { sb1ParCon === undefined ? 'Data should go here' : sb1ParCon } ~ { appNamStr }
